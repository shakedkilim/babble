...
var Babble = { // Data object to save all chat logs and methods
...
//require('../client/scripts/main.js');

//var Babble = Module.Babble;
        ...
        if (!url.path) {
            response.writeHead(400);
                ...
                if( Babble.users.includes(name) )
                {
                        ...
                        (Babble.users).splice( name , 1 );
                        Babble.userCount = (Babble.users).length; // update user count
                        response.writeHead(200);
                }


        } else if( (url.path).includes("messages") ){
                
                if( ( parseInt( (url.path).substring( (url.path).indexOf('=')+1, (url.path).length ) ) != (Babble.messages).length ) && (url.path).includes("messages?counter=") )
                {
 ...
 * @param {*} time 
 * @param {*} mail 
 * @param {*} link : photo link
 */
function message(name,text,time,mail,link){
...
server.listen(9000);
setInterval(function(){Babble.users=[];Babble.userCount=0;},15000); // clear to refresh data
